...
abstract class AbstractCollection extends ArrayIterator implements CollectionInterface
{
	...
	{
		$newCollection = $this->newInstance();
		$this->each(function(AbstractBean $collectable) use($newCollection, $collection){
...
// 			if($this->containsIndex($collectable->getIndex()))
	...
	public function filter($callable)
	{
		...
		$newCollection = $this->newInstance();
		$this->each(function(AbstractBean $collectable) use($newCollection, $callable){
	...
	public function foldLeft($start, $callable)
	{
		$this->validateCallback($callable);
		$result = $start;
		$this->each(function(AbstractBean $collectable) use(&$result, $callable){
	...
	public function forall($callable)
	{
		...
		$this->validateCallback($callable);
		return $this->foldLeft(true, function($boolean, AbstractBean $collectable) use($callable){
	...
	public function partition($callable)
	{
		...
		$this->each(function(AbstractBean $collectable) use($getCollection, $callable){
